...
export function SearchBar({ className }: SearchBarProps) {
  ...
  const menuData: SearchItem[] = menuItems.map(item => ({
    id: item.id,
    name: item.name,
    nameAr: item.nameAr,
  ...
  const { results, search, clearSearch, hasResults, isSearching } = useSearch(menuData);
          ...
          {isSearching ? (
            ...
            <div className="p-6 text-center">
              ...
              <p className="mt-1 text-xs text-text-muted">
                {isAr ? "جرب كلمات مختلفة" : "Try different keywords"}
              </p>
            </div>
          )}
        </div>
...
// Quick search shortcut component
export function QuickSearch() {
  const locale = useLocale();
  const isAr = locale === "ar";
  
  const popularSearches = [
    { en: "Wagyu Burger", ar: "برجر واغيو", query: "wagyu" },
    { en: "Seafood", ar: "مأكولات بحرية", query: "seafood" },
    { en: "Pasta", ar: "باستا", query: "pasta" },
    { en: "Salmon", ar: "سلمون", query: "salmon" },
  ];
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {popularSearches.map((item) => (
        <Link
          ...
          className="rounded-full bg-surface-card px-3 py-1 text-xs text-text-secondary transition-colors hover:bg-brand hover:text-white"
        >
