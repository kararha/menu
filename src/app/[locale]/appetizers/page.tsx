...
export default function AppetizersPage() {
  ...
  const {
    ...
    initialPage: 1,
    initialLimit: 9,
    initialCategory: "appetizers",
  });

  const sidebarCategories = [
    { id: "appetizers", label: isAr ? "جميع المقبلات" : "All Appetizers" },
    { id: "appetizers-soups", label: isAr ? "الشوربات" : "Soups" },
    { id: "appetizers-salads", label: isAr ? "السلطات" : "Salads" },
    { id: "appetizers-seafood", label: isAr ? "المأكولات البحرية" : "Seafood" },
    { id: "appetizers-cheese", label: isAr ? "الأجبان" : "Cheese" },
  ];
        ...
        <aside className="hidden w-56 shrink-0 lg:block">
          <nav className="flex flex-col gap-1">
            {sidebarCategories.map((cat) => (
              <Link
                key={cat.id}
                href={`/${locale}/appetizers?sub=${cat.id}`}
        ...
        <main className="flex-1 ps-0 lg:ps-8">
          <FadeIn>
            <div className="mb-4 flex items-center gap-2 text-xs text-text-muted">
              ...
              <ChevronRight className="h-3 w-3" />
              <span className="text-text-secondary">{isAr ? "المقبلات" : "Appetizers"}</span>
            </div>

            <h1 className="mb-1 text-3xl font-bold text-white">{isAr ? "المقبلات" : "Appetizers"}</h1>
            <p className="mb-6 text-sm text-text-secondary">
              {isAr ? "ابدأ وجبتك بمقبلات لذيذة" : "Start your meal with delicious appetizers"}
            </p>
          </FadeIn>
                      ...
                      <div className="relative h-48 overflow-hidden">
                        ...
                        {item.badge && (
                          <span className={`absolute start-3 top-3 rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white ${
                            ...
                            item.badgeColor === "gold" ? "bg-amber-500" : "bg-orange-500"
                          }`}>
