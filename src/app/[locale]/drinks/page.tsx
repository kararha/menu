...
export default function DrinksPage() {
  ...
  const sidebarCategories = [
    { id: "drinks", label: isAr ? "جميع المشروبات" : "All Drinks" },
    { id: "drinks-cocktails", label: isAr ? "الكوكتيلات" : "Cocktails" },
    { id: "drinks-wine", label: isAr ? "النبيذ" : "Wine" },
    { id: "drinks-beer", label: isAr ? "الجعة" : "Beer" },
    { id: "drinks-non-alcoholic", label: isAr ? "بدون كحول" : "Non-Alcoholic" },
  ];
        ...
        <aside className="hidden w-56 shrink-0 lg:block">
          <nav className="flex flex-col gap-1">
            {sidebarCategories.map((cat) => (
              <Link key={cat.id} href={`/${locale}/drinks?sub=${cat.id}`} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-start text-sm font-medium text-text-secondary transition-all hover:bg-surface-card hover:text-white">
                {cat.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="mt-6 rounded-xl border border-border-subtle bg-surface-card p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-sm font-semibold text-white">{isAr ? "ساعة السعادة" : "Happy Hour"}</h3>
            </div>
            <p className="mt-1 mb-3 text-xs text-text-secondary">
              {isAr ? "خصم 30% على المشروبات من 4-7 مساءً" : "30% off drinks from 4-7 PM"}
            </p>
            <button className="text-sm font-semibold text-brand transition-colors hover:text-brand-light">{isAr ? "شاهد التوقيتات" : "View Times"}</button>
          </div>
        </aside>
        ...
        <main className="flex-1 ps-0 lg:ps-8">
          <FadeIn>
            <div className="mb-4 flex items-center gap-2 text-xs text-text-muted">
              ...
              <ChevronRight className="h-3 w-3" />
              <span className="text-text-secondary">{isAr ? "المشروبات" : "Drinks"}</span>
            </div>
            <h1 className="mb-1 text-3xl font-bold text-white">{isAr ? "المشروبات وال wines" : "Drinks & Wines"}</h1>
            <p className="mb-6 text-sm text-text-secondary">{isAr ? "م cocktails ونبيذ مميز" : "Signature cocktails and fine wines"}</p>
          </FadeIn>
                  ...
                  <StaggerItem key={item.id}>
                    <Link href={`/${locale}/item?id=${item.id}`} className="group overflow-hidden rounded-xl border border-border-subtle bg-surface-card transition-all hover:border-brand/40 hover:shadow-lg hover:shadow-brand/5">
                      <div className="relative h-48 overflow-hidden">
                        ...
                        {item.badge && <span className={`absolute start-3 top-3 rounded px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white ${item.badgeColor === "blue" ? "bg-blue-600" : item.badgeColor === "green" ? "bg-green-600" : "bg-amber-500"}`}>{item.badge}</span>}
