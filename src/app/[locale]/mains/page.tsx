      ...
      <div className="mx-auto flex max-w-7xl gap-0 px-6 py-6">
        <aside className="hidden w-56 shrink-0 lg:block">
          <nav className="flex flex-col gap-1">
            {sidebarCategories.map((cat) => (
              <Link
                key={cat.id}
                href={`/${locale}/mains?sub=${cat.id}`}
                className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-start text-sm font-medium text-text-secondary transition-all hover:bg-surface-card hover:text-white"
              >
                {cat.label}
              </Link>
            ))}
        ...
        <main className="flex-1 ps-0 lg:ps-8">
          ...
          <p className="mb-6 text-sm text-text-secondary">
            {isAr ? "تشكيلة من الأطباق الرئيسية الفاخرة" : "A selection of premium main courses"}
          </p>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {item.spicyLevel && item.spicyLevel > 0 && (
                            <div className="flex items-center gap-0.5">
                              {Array.from({ length: item.spicyLevel }).map((_, i) => <Flame key={i} className="h-3 w-3 text-orange-500" />)}
                            </div>
                          )}
                        </div>
